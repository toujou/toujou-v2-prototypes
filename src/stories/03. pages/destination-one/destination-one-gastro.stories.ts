...
export default {
    ...
    argTypes: {
        mapAspectRatio: {
            table: {
                category: "D1I Gastro Page Settings",
...
interface DestinationOneGastroProps {
...
const renderMain = (args: DestinationOneGastroProps) => {
    return `
        <main id="main" skippy-links-label="Main content">
            ${renderBreadcrumbExampleBlock()}
            <div class="fragment" fragment-type="d1i-gastro" map-aspect-ratio="${args.mapAspectRatio}">
                ${renderDi1PageIntroSection('GASTRO')}
                ${renderPageFragment()}
                ${renderMapSection()}
            </div>
            ${renderPageContent()}
...
const Template: StoryFn<DestinationOneGastroProps> = (args: DestinationOneGastroProps)=> {
    return `
       ${renderMain(args)}
    `;
};
