...
describe('event table [mobile]', () => {
    ...
    it.only('has correct styles', () => {
        ...
        cy.get(testEventCardSelector).should('have.css', 'grid-template-areas', '"calendarPages eventContent eventChip"');
        cy.get(testEventCardSelector).should('have.css', 'grid-template-columns', '128px 696.898px 87.1016px');
        ...
        cy.get(`${testEventCardSelector} .event-table-card__content`).should('have.css', 'width', '696.8984375px');
        cy.get(`${testEventCardSelector} .event-table-card__content`).should('have.css', 'height', '29px');
