...
describe('average rating', () => {
    ...
    it.only('has correct styles', () => {
        ...
        cy.get(`${avrRatingSelector} .average-rating__rating`).should('have.css', 'grid-template-columns', '28.75px 108px');
        cy.get(`${avrRatingSelector} .average-rating__rating`).should('have.css', 'width', '144.75px');
        ...
        cy.get(`${avrRatingSelector} .average-rating__link`).should('have.css', 'text-decoration', `underline`);
    });
})
