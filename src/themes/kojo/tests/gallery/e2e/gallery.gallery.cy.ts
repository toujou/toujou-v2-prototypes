...
describe('gallery [mobile]', () => {
    ...
    it('has correct styles', () => {
        ...
        cy.get('.gallery').should('have.css', 'grid-template-columns', '296px');
        cy.get('.gallery').should('have.css', 'grid-gap', `${tokens.spacing.s}`);
        cy.get('.gallery').should('have.css', 'width', '296px');
        cy.get('.gallery').should('have.css', 'max-width', '960px');
        cy.get('.gallery').should('have.css', 'margin', '0px 16px');
        ...
        cy.get('.gallery .gallery__item:first-child .gallery__image').should('have.css', 'height', '182.9375px');
        cy.get('.gallery .gallery__item:first-child .gallery__image').should('have.css', 'width', '296px');
    ...
    it('has correct hover styles', () => {
        ...
        // @ts-ignore
        cy.get('.gallery .gallery__item:first-child').before('height').should('eq', '182.938px');
        // @ts-ignore
        cy.get('.gallery .gallery__item:first-child').before('width').should('eq', '296px');
        // @ts-ignore
        ...
        // @ts-ignore
        cy.get('.gallery .gallery__item:first-child').after('top').should('eq', '91.4688px');
        // @ts-ignore
        cy.get('.gallery .gallery__item:first-child').after('left').should('eq', '148px');
        // @ts-ignore
